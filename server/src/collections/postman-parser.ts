import { HttpMethod } from '../prisma/http-method.enum';
...
const METHOD_MAP = new Set<keyof typeof HttpMethod>([
  'GET',
  'POST',
  'PUT',
  'PATCH',
...
function toEnumMethod(m?: string): HttpMethod | null {
  ...
  return (HttpMethod as any)[up] && METHOD_MAP.has(up as any) ? (HttpMethod as any)[up] : null;
}
...
function walk(items: PostmanItem[] | undefined, parentPath: string[], out: ParsedRequest[]) {
  ...
  for (const it of items) {
    ...
    if (it?.request) {
      ...
      if (!m) {
        if (it.request.method) console.warn(`unsupported method: ${it.request.method}`);
